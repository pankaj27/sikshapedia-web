...
import { FiArrowLeft, FiSave, FiImage, FiList, FiGrid, FiSettings, FiSearch, FiPlus, FiTrash2, FiEye, FiUpload, FiDollarSign, FiCalendar } from 'react-icons/fi';
...
const SCHOLARSHIP_TYPES = ['Merit-Based', 'Need-Based', 'Sports Quota', 'Minority', 'Government', 'Private', 'International'];
const EDUCATION_LEVELS = ['Undergraduate', 'Postgraduate', 'PhD', 'Diploma', 'School', 'All Levels'];
...
const ScholarshipForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    provider: '',
    provider_type: 'Government', // Government, Private, NGO, Corporate
    
    // Amount & Dates
    amount: '',
    amount_type: 'Fixed', // Fixed, Variable, Full Tuition
    currency: 'INR',
  ...
  useEffect(() => {
    if (formData.auto_generate_seo) {
      ...
      if (!formData.meta_title && formData.name) {
        autoSeo.meta_title = `${formData.name} | Scholarship Details | Admissionbuddy`;
      }
      ...
      if (!formData.featured_image_alt && formData.name) {
        autoSeo.featured_image_alt = `${formData.name} Scholarship | Admissionbuddy`;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await api.put(`/scholarships/${id}`, formData);
      } else {
          ...
          <form onSubmit={handleSubmit} className="max-w-4xl space-y-6">
            ...
            {activeTab === 'basic' && (
              <div className="space-y-6">
                <div className="bg-white rounded-lg shadow-sm border p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium mb-1">Scholarship Type *</label>
                        <select
                    ...
                    <div className="flex items-center gap-4">
                      <label className="flex items-center gap-2">
                        <input
                          ...
                          className="rounded"
                        />
                        <span className="text-sm">Featured Scholarship</span>
                      </label>
                      <label className="flex items-center gap-2">
                        <input
                          type="checkbox"
                ...
                {/* Amount & Dates */}
                <div className="bg-white rounded-lg shadow-sm border p-6">
                  <h2 className="text-lg font-semibold mb-4">💰 Amount & Important Dates</h2>
                  <div className="space-y-4">
              ...
              <div className="space-y-6">
                <div className="bg-white rounded-lg shadow-sm border p-6">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        <label className="block text-sm font-medium mb-1">Minimum Percentage</label>
                        <input
                          type="text"
                          value={formData.min_percentage}
                          onChange={(e) => handleChange('min_percentage', e.target.value)}
                          className="w-full border rounded-lg px-4 py-2.5"
                          placeholder="e.g., 60%"
                        />
                      </div>
                      <div>
                        <label className="block text-sm font-medium mb-1">Max Family Income</label>
                        <input
                          type="text"
                          value={formData.max_family_income}
                          onChange={(e) => handleChange('max_family_income', e.target.value)}
                          className="w-full border rounded-lg px-4 py-2.5"
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        <label className="block text-sm font-medium">Eligibility Points</label>
            ...
            {activeTab === 'media' && (
              <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <button
                          type="button"
                          onClick={() => handleChange('featured_image_alt', `${formData.name} Scholarship | Admissionbuddy`)}
                ...
                <div className="bg-white rounded-lg shadow-sm border p-6">
                  <h2 className="text-lg font-semibold mb-4">🏢 Provider Logo</h2>
                  <input
                    type="url"
                    value={formData.provider_logo}
                    onChange={(e) => handleChange('provider_logo', e.target.value)}
                    className="w-full border rounded-lg px-4 py-2.5"
                    placeholder="Provider logo URL"
                  />
                  {formData.provider_logo && (
                    <img src={formData.provider_logo} alt="Provider Logo" className="mt-3 h-16 object-contain" />
                  )}
                </div>
              </div>
            )}
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <textarea
                        ...
                        rows="12"
                        placeholder="<h2>About the Scholarship</h2><p>...</p>"
                      />
                    </div>
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-1">Selection Process</label>
                      <textarea
                ...
                <div className="bg-white rounded-lg shadow-sm border p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <select
                        value={formData.schema_type || 'Scholarship'}
                ...
                <div className="bg-white rounded-lg shadow-sm border p-6">
                  ...
                  <div className="border rounded-lg p-4 bg-gray-50">
                    <p className="text-blue-600 text-lg hover:underline cursor-pointer">
                      {formData.meta_title || formData.name || 'Scholarship Title'}
                    </p>
                    <p className="text-green-700 text-sm">
                      {window.location.origin}/scholarships/{formData.slug || 'slug'}
                    </p>

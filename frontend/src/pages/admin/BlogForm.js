...
const CATEGORIES = ['Career Guidance', 'Study Tips', 'Exam Preparation', 'College Life', 'Industry Insights', 'Success Stories', 'Education News'];
...
const BlogForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    show_related_exams: false,
    show_related_colleges: false,
    show_newsletter: true,
    show_cta_banner: false,
    cta_banner: { title: '', subtitle: '', button_text: '', button_link: '', gradient: 'from-blue-500 to-purple-500' },
  ...
  useEffect(() => {
    if (formData.auto_generate_seo && formData.title) {
      ...
      const newMetaTitle = `${formData.title} | Blog | Admissionbuddy`;
      if (formData.meta_title !== newMetaTitle) {
        autoSeo.meta_title = newMetaTitle;
      }
      // Update meta_description from excerpt
      if (formData.excerpt) {
        const newMetaDesc = formData.excerpt.slice(0, 160);
        if (formData.meta_description !== newMetaDesc) {
          autoSeo.meta_description = newMetaDesc;
        }
      }
      // Update alt tag
      const newAlt = `${formData.title} | Blog | Admissionbuddy`;
  ...
  }, [formData.title, formData.excerpt, formData.featured_image, formData.auto_generate_seo]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        await api.put(`/blogs/${id}`, dataToSave);
      } else {
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (formData.title) {
                              const autoAlt = `${formData.title} - ${formData.category} | Admissionbuddy Blog`;
                  ...
                  <div className="space-y-4">
                    ...
                    {formData.video_url && (
                      <div className="aspect-video">
                        <iframe src={formData.video_url} className="w-full h-full rounded-lg" allowFullScreen />
                      </div>
                    )}
                    <div>
                  ...
                  <div className="space-y-3">
                    <label className="flex items-center gap-3 p-3 border rounded-lg hover:bg-gray-50">
                      <input
                        ...
                        className="rounded"
                      />
                      <span>Show Related Blog Posts</span>
                    </label>
                    <label className="flex items-center gap-3 p-3 border rounded-lg hover:bg-gray-50">
                      <input
                        type="checkbox"
                ...
                <div className="bg-white rounded-lg shadow-sm border p-6">
                  ...
                  <div className="border rounded-lg p-4 bg-gray-50">
                    ...
                    <p className="text-green-700 text-sm">
                      {window.location.origin}/blog/{formData.slug || 'slug'}
                    </p>

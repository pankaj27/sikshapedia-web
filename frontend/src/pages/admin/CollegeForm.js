...
const CollegeForm = () => {
  ...
  const fetchCollege = async () => {
    ...
    try {
      ...
      const normalizedData = {
        ...
        campus_size: collegeData.campus_size || '',
        total_students: collegeData.total_students || 0,
        average_fees: collegeData.average_fees || 0,
        // Rankings
        nirf_ranking: collegeData.nirf_ranking || null,
        india_today_ranking: collegeData.india_today_ranking || null,
        outlook_ranking: collegeData.outlook_ranking || null,
        // Website & contact
        ...
        logo_title: collegeData.logo_title || '',
        logo_alt: collegeData.logo_alt || '',
        banner_url: collegeData.banner_url || '',
        banner_title: collegeData.banner_title || '',
        banner_alt: collegeData.banner_alt || '',
        brochure_url: collegeData.brochure_url || '',
        campus_video_url: collegeData.campus_video_url || '',
        campus_video_title: collegeData.campus_video_title || '',
        campus_video_description: collegeData.campus_video_description || '',
        virtual_tour_url: collegeData.virtual_tour_url || '',
        virtual_tour_title: collegeData.virtual_tour_title || '',
        virtual_tour_description: collegeData.virtual_tour_description || '',
        // Content
        description: collegeData.description || '',
        admission_process: collegeData.admission_process || '',
        // Badges
        is_verified: collegeData.is_verified || false,
        is_preferred: collegeData.is_preferred || false,
        is_featured: collegeData.is_featured || false,
        is_trending: collegeData.is_trending || false,
        is_top_rated: collegeData.is_top_rated || false,
        is_sponsored: collegeData.is_sponsored || false,
        is_admission_partner: collegeData.is_admission_partner || false,
        is_no_cost_emi: collegeData.is_no_cost_emi || false,
  ...
  // Helper function to save a single section's data (used by both handleSectionSave and handleSequentialSaveAll)
    ...
    switch (section) {
      case 'basic':
        return {
          ...
          classes_offered: formData.classes_offered || [],
  ...
  const handleSequentialSaveAll = async (targetStatus) => {
    if (!id) {
      toast({
        title: "⚠️ Error",
        description: "Please save the form as draft first.",
        variant: "destructive"
      });
      return false;
    }

    const sections = ['basic', 'media', 'courses', 'details', 'admission', 'seo-content'];
    let allSaved = true;
    ...
    if (allSaved) {
      ...
      try {
        await api.patch(`/colleges/${id}/section/basic`, { status: targetStatus });
        toast({
          title: targetStatus === 'published' ? "✅ Published!" : "✅ Submitted for Review!",
          description: targetStatus === 'published' 
            ? "All sections saved and college published successfully!" 
                  ...
                  {recognitions.length === 0 ? (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      {recognitions
                        .filter(recognition => 
                          !recognitionSearch || 
                          recognition.name.toLowerCase().includes(recognitionSearch.toLowerCase()) ||
                          (recognition.full_name && recognition.full_name.toLowerCase().includes(recognitionSearch.toLowerCase()))
                        )
                        .map((recognition) => {

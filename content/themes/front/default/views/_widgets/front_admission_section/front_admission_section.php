...
<?php
if(!empty($college_admission_data)){
	?>
	<div class="card infoCard mb-4">
	  ...
	  <div class="card-body">
	  	...
	  	if(!empty($college_admission_intro)){
	  		echo $college_admission_intro;
	  	}

	  	if(!empty($college_admission_data)){
	  		foreach ($college_admission_data as $key => $value) {
	  			if( in_array($value->info_value_type, array('general','ads','image'))){
	  	...
	  	<?php
		  if(!empty($college_admission_faqs)){
		    ?>
		    <div id="accordionExample1" class="accordion">
		      <?php
		      $i=0;
		      foreach ($college_admission_faqs as $key => $value) {
		        ?>
		        <div class="card br-0">
		          <a href="#" data-toggle="collapse" data-target="#collapse<?php echo $i;?>" aria-expanded="true" aria-controls="collapse<?php echo $i;?>" class="card-header d-block position-relative text-dark text-uppercase collapsible-link "><strong></strong><?php echo $value->faq_question;?></a>
		          <div id="collapse<?php echo $i;?>" data-parent="#accordionExample1" class="collapse <?php echo ($i==0)?'show':'';?>">
		            <div class="card-body">
		              <p class="font-weight-light m-0"><strong></strong><?php echo $value->faq_ans;?></p>
		            </div>
		          </div>
		        </div>
		        <?php

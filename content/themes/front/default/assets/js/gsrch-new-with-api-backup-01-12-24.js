...
window.addEventListener('scroll', () => {
  ...
  if (currentScrollPos > prevScrollPos) {
    ...
    if (currentScrollPos + window.innerHeight >= collegesListContainer.offsetHeight) {
      // Check if an API request is not in progress before initiating the next one
      if (!isFetching) {
        // Set the flag to indicate that an API request is in progress
        isFetching = true;
        // Increment data_start and fetch data
        data_start += data_length;
        console.log('data_start:'+data_start);
        apiUrl = "https://staging.sikshapedia.com/api/listing?data=" + _pgid + "&length=" + data_length + "&start=" + data_start;
  ...
  try{
    ...
    console.log('hasNext:'+hasNext)
      ...
      if (hasNext && data_start < dataCount) {
        ...
        isFetching = false;
        //await fetchDataAndRender(data_start); // Trigger the next API call
      } else {

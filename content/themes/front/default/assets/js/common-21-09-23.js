...
if($('#applicant_country').length>0){
    ...
    $.ajax({
      type:'GET',
      url:base_url+'get_countries?_c='+_co,
      data:{},
      beforeSend: function(){ 
      ...
      success:function(d){
        $.each(d,function(k,v){
          html+='<option value="'+v['country_id']+'#'+v['country_code']+'" data-ccode="'+v['country_code']+'" '+v['selected']+'>'+v['country_name']+'</option>';
        });
       ...
       loadstates($("#applicant_country :selected").val());
...
//$.lockfixed(".sticky-item",{offset: {top: 70, bottom: 100}}); 
  ...
  function loadstates_old(c){
    ...
    // $.ajax({
    //   type:'GET',
    //   url:wb_api+'coapi/country_states',
    //   data:{},
    //   beforeSend: function(){ 

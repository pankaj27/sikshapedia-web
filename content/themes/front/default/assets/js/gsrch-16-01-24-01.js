...
function renderData(data) {
  ...
  console.log('slugData:'+slugData.length);
...
function handleRadioChange(event) {
  ...
  if (linkValue) {
    ...
    if (parameterExists) {
      ...
      var updatedFilterapiUrl = filterapiUrl.replace(regex, `$1${filterParamData}`);
    } else {
      // Append the new parameter to filterapiUrl
      var separator = filterapiUrl.includes('?') ? '&' : '?';
      var updatedFilterapiUrl = filterapiUrl + separator + `${filterParam}=${filterParamData}`;
    }

    filterListContainer.innerHTML = '';

    // Call the loadData function with the updated URL
    loadData(updatedFilterapiUrl);

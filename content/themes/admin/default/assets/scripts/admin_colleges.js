jQuery(function($) {
  ...
  if($('#colleges_same_group_colleges').length>0){
    $('#colleges_same_group_colleges').chosen({no_results_text: "Select Other Colleges In this Group"});
  }

  if($('#college_gallery_category').length>0){
    $('#college_gallery_category').chosen();
  }

  if($('#top_colleges').length>0){
    $('#top_colleges').chosen();
  }

  if($('#college_inner_menu_other_link').length>0){
    $('#college_inner_menu_other_link').chosen();
  }

  if($('select[name=college_faculty_designation]').length>0){
    $('select[name=college_faculty_designation]').chosen();
  }


  if($('select[name=college_faculty_department]').length>0){
    $('select[name=college_faculty_department]').chosen();
  }

  if($('#college_has_leads_area_dist').length>0){
    $('#college_has_leads_area_dist').chosen();
  }

  if($('#college_leads_type').length>0){
    $('#college_leads_type').chosen();
  }


  if($('#college_leads_coursee').length>0){
    $('#college_leads_coursee').chosen();
  }


  if($('#college_menu_widgets').length>0){
    $('#college_menu_widgets').chosen();
  }

  if($('#search_college_country').length>0){
  ...
  $('body').on('change','#college_leads_type',function(){
    var leads_streams=$(this).val();

    $.ajax({
      type:'POST',
      url:base_url+'/institutions/get_leads_course_list',
      data:{[csrf_name]:csrf_hash,leads_streams:leads_streams},
      success:function(d){
        var html='';
        if(d.courses!=''){
          $.each(d.courses,function(i,v){
            html+='<option value="'+v.course_id+'">'+v.course_name+'</option>';
          });
        }else{
          html='';
        }
        $('#college_leads_coursee').html(html);
        $('#college_leads_coursee').trigger("chosen:updated");
      }
    });
  });
        ...
        $.ajax({
          ...
          success:function(d){
            if(d.structure_data){
              ...
              pre_elem.innerHTML =d.structure_data;// prettyPrintJson.toHtml(d.structure_data);
            }else{
              alert(d.error);
            }
          }
...
function tiny_mce_old(ctrl_area){
    tinymce.init({
      selector: ctrl_area,
      license_key: 'gpl',
      entity_encoding : "raw",
...
function jodit(ctrl_area){
  ...
  $('#collegeInnerMenuesEditModal').on('hidden.bs.modal', function () {
    ...
    if ($('#college_course_eligibility_broad').length > 0) {
      const college_course_eligibility_broad = Jodit.make('#college_course_eligibility_broad');
      ...
      $('#form_college_courses_fees_intro').validate({
          ...
          submitHandler: function () {
            ...
            var formData = new FormData($('#form_college_courses_fees_intro')[0]);

            // Get Jodit editor content
            var editorContent = college_course_eligibility_broad.value(); // Fetch the content

            console.log('Editor Content:', editorContent); // Debugging the editor content

            // Append editor content to FormData
            formData.append('college_course_eligibility_broad', editorContent);
 ...
 $("#college_faculty_subjects").chosen({no_results_text: "Select Subjects"});
  ...
  if($('#college_placement_info').length>0){
    ...
    tiny_mce('#college_placement_info');
  }

  if($('#college_about_info').length>0){
    //jodit('#college_about_info');
    tiny_mce('#college_about_info');
  }

...
class Front_search_colleges_filter extends Widget
{
	...
	function run($visible = FALSE) {
    	...
    	$country_states=$this->com->__get_states('state_id,state_name,state_name_slug',array('state_country_id'=>$country_id,'state_visible_in_filter'=>'yes'),'state_serial','ASC');

    	if(isset($slug_3)){
    		if(!empty($slug_data_3)){
    			if($slug_data_3->slug_type=='1'){
					$country_cities=$this->com->__get_city('city_id,city_name,city_name_slug,city_name_state_slug',array('city_country_id'=>$country_id,'city_state_id'=>$slug_data_3->slug_type_id,'city_status'=>'1','city_visible_in_filter'=>'yes'),FALSE,'city_serial','ASC');
    			}else{
    				$country_cities=$this->com->__get_city('city_id,city_name,city_name_slug,city_name_state_slug',array('city_country_id'=>$country_id,'city_status'=>'1','city_visible_in_filter'=>'yes'),FALSE,'city_serial','ASC');
    			}
		...
		if(!empty($country_cities)){
			foreach ($country_cities as $key => $value) {
				...
				$_city_selected=(isset($slug_4) && ($value->city_name_slug==$slug_4))?'checked':'';
				...
				$city_slug=$college_srch_base_url.'/'.$value->city_name_state_slug;
		...
		$affiliation_types=$this->im->__get_affiliation_types('statutory_body_id,statutory_body_abbr,statutory_body_name,statutory_body_abbr_slug',array('statutory_body_status'=>'1','statutory_body_country_id'=>$country_data->country_id),FALSE,'statutory_body_abbr','ASC');
		...
		$streams=$this->strm->__get_stream('stream_id,stream_name',array('stream_status'=>'1','stream_visible_in_filter'=>'yes'),FALSE,'stream_serial','ASC');
		if(!empty($streams)){
		...
		if(!empty($agencies)){
			...
			foreach ($agencies as $key => $value) {
				$currentUrl.='agn='.$value->agency_name_slug;
				$_agencies[]=array(
					'agency_id'=>encode_data($value->agency_id),
					'agency_short_name'=>$value->agency_short_name,
					'agency_selected'=>'',
					'url_slug'=>$currentUrl.'?agn='.$value->agency_name_slug
				);
			}

...
class Front_menu_bottom extends Widget
{
	function run($visible = FALSE){
    	...
    	$menues=$this->sm->get_menues(array('menu_is_top'=>'2','menu_is_footer'=>'1','menu_is_active'=>'1','menu_parent_id'=>'0'),FALSE,'menu_serial','ASC',FALSE);

    	$main_menues=array();

    	if(!empty($menues)){
    		foreach ($menues as $key => $value){
    			$sub_menues=$this->sm->get_menues(array('menu_is_top'=>'2','menu_is_active'=>'1','menu_is_footer'=>'1','menu_parent_id'=>$value->menu_id),FALSE,'menu_serial','ASC',FALSE);

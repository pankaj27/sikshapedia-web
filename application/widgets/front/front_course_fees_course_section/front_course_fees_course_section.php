...
class Front_course_fees_course_section extends Widget
{
	...
	function run($visible = FALSE,$ids=null){
    	...
    	$_course_data=$this->im->_get_user_course_data(array('user_id'=>$college_id,'course_id'=>$slug_data->slug_type_id,'user_type'=>'4'),NULL,TRUE);
    		...
    		foreach ($cost_group as $key => $value) {
    			...
    			if($value->user_course_cost_type==1 && $value->user_course_cost_breakup_type==1){
    				...
    				foreach ($fees_data as $k => $v) {
    					if($v->user_course_admisssion_fee_total!=NULL || $v->user_course_admisssion_fee_total>0){
    						$total+=$v->user_course_tution_fee_total+$v->user_course_admisssion_fee_total+$v->user_course_reg_fee_sem_1+$v->user_course_exam_fee_sem_1+$v->user_course_other_fee_sem_1+$v->user_course_other_fee_total;
    					}else{
    						$total+=$v->user_course_tution_fee_total+$v->user_course_admisssion_fee_sem_1+$v->user_course_reg_fee_sem_1+$v->user_course_exam_fee_sem_1+$v->user_course_other_fee_sem_1+$v->user_course_other_fee_total;
    					}

    					if($v->user_course_admisssion_fee_total!=NULL || $v->user_course_admisssion_fee_total>0){
    						if($v->user_course_reg_fee_total>0){
    							$year_total=$v->user_course_tution_fee_total+$v->user_course_admisssion_fee_total+$v->user_course_reg_fee_total+$v->user_course_exam_fee_sem_1+$v->user_course_other_fee_sem_1+$v->user_course_other_fee_total;
    						}else{
    							$year_total=$v->user_course_tution_fee_total+$v->user_course_admisssion_fee_total+$v->user_course_reg_fee_sem_1+$v->user_course_exam_fee_sem_1+$v->user_course_other_fee_sem_1+$v->user_course_other_fee_total;
    						}    						
    					}else{
    						if($v->user_course_reg_fee_total>0){
    							$year_total=$v->user_course_tution_fee_total+$v->user_course_admisssion_fee_sem_1+$v->user_course_reg_fee_total+$v->user_course_exam_fee_sem_1+$v->user_course_other_fee_sem_1+$v->user_course_other_fee_total;
    						}else{
    							$year_total=$v->user_course_tution_fee_total+$v->user_course_admisssion_fee_sem_1+$v->user_course_reg_fee_sem_1+$v->user_course_exam_fee_sem_1+$v->user_course_other_fee_sem_1+$v->user_course_other_fee_total;
    						}
    					...
    					if($v->user_course_admisssion_fee_total!=NULL || $v->user_course_admisssion_fee_total>0){
    						...
    						if($v->user_course_reg_fee_total>0){
    							$ft=$v->user_course_tution_fee_total+$v->user_course_admisssion_fee_total+$v->user_course_reg_fee_total+$v->user_course_exam_fee_sem_1+$v->user_course_other_fee_sem_1+$v->user_course_other_fee_total;
    						}else{
    							$ft=$v->user_course_tution_fee_total+$v->user_course_admisssion_fee_total+$v->user_course_reg_fee_sem_1+$v->user_course_exam_fee_sem_1+$v->user_course_other_fee_sem_1+$v->user_course_other_fee_total;
    						}
    						...
    						if($v->user_course_reg_fee_total>0){
    							$ft=$v->user_course_tution_fee_total+$v->user_course_admisssion_fee_sem_1+$v->user_course_reg_fee_total+$v->user_course_exam_fee_sem_1+$v->user_course_other_fee_sem_1+$v->user_course_other_fee_total;
    						}else{
    							$ft=$v->user_course_tution_fee_total+$v->user_course_admisssion_fee_sem_1+$v->user_course_reg_fee_sem_1+$v->user_course_exam_fee_sem_1+$v->user_course_other_fee_sem_1+$v->user_course_other_fee_total;
    						}
    					...
    					$_fees_data[$value->user_course_cost_category][]=array(
    						...
    						'fees_admission_total['.$v->user_course_year.']'=>number_format($v->user_course_admisssion_fee_total),
    						'fees_registration_sem_1['.$v->user_course_year.']'=>number_format($v->user_course_reg_fee_sem_1),
    						'fees_registration_fees_total['.$v->user_course_year.']'=>number_format($v->user_course_reg_fee_total),
    						...
    						'fees_other_sem_1['.$v->user_course_year.']'=>($v->user_course_other_fee_sem_1>0)?number_format($v->user_course_other_fee_sem_1):number_format($v->user_course_other_fee_total),
    				...
    				$fees_data=$this->im->get_user_course_grand_total(array('user_id'=>$college_id,'user_course_cost_category'=>$value->user_course_cost_category,'user_course_id'=>$slug_data->slug_type_id));
    			...
    			$cost_data['fees_duration_text']=$fees_duration_text;
    		}
    	}
    	...
    	$course_data=array(
			...
			'course_streams'=>'',
			'course_total_sems'=>'',
			'course_seats_available'=>(!empty($_course_data->user_course_total_seats))?$_course_data->user_course_total_seats:'',

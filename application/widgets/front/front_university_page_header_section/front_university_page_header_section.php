 ...
 * 
 */
class Front_university_page_header_section extends Widget
{
	...
	function run($visible = FALSE) {
        ...
        $college_data=$this->im->get_university_profile_data(array('university_user_id'=>$user_id));
        $college_type=$this->im->get_institute_types(array('inst_type'=>$college_data->university_type));

        $country_data=$this->com->get_country(array('country_id'=>$college_data->university_country_id));
        $city_data=$this->com->get_city(array('city_id'=>$college_data->university_city_id));
        $state_data=$this->com->get_state(array('state_id'=>$college_data->university_state_id));

        $uni_slug=$this->sm->get_slug(array('slug_type'=>'7','slug_type_id'=>$college_data->university_user_id));
        ...
        if($segment_3!='0'){
            ...
            if(!empty($segment_3_details->menu_name_alias) || $segment_3_details->menu_name_alias!=NULL){
                $college_name_formatted=strtoupper($college_data->university_name).','.strtoupper($city_data->city_name).' - '.strtoupper($segment_3_details->menu_name_alias);
                $breadcumb=array(
                    'HOME'=>base_url(),
                ...
                $college_name_formatted=strtoupper($college_data->university_name).','.strtoupper($city_data->city_name);
                $breadcumb=array(
                    'HOME'=>base_url(),
                    strtoupper($city_data->city_name)=>base_url().strtolower($country_data->country_iso_code_2).'/colleges/'.$state_slug->slug_value.'/'.$city_slug->slug_value,
                    strtoupper($segment_3_details->menu_name)=>''
        ...
        if(!empty($college_data->university_affiliation_type)){
            $caffiliations=$this->im->get_group_concat_affiliation_types('statutory_body_abbr','statutory_body_id',$college_data->university_affiliation_type);
        ...
        $data['college_data']=array(
            'college_id'=>encode_data($college_data->university_id),
            'college_user_id'=>encode_data($college_data->university_user_id),
            'college_name'=>strtoupper($college_data->university_name),
            ...
            'college_country_id'=>encode_data($college_data->university_country_id),
            ...
            'college_estd'=>$college_data->university_estd_year,
        ...
        if ($visible) $this->render('front_university_page_header_section',$data);
    }
}
...
class Front_course_fees_section extends Widget
{
	function run($visible = FALSE){
        ...
        if((is_string($segment_1) && $segment_1!='0') && (is_string($segment_2) && $segment_2!='0') || (is_string($segment_3) && $segment_3!='0' && $segment_3=='courses-fees')){
						...
						if(!empty($_college_data)){
							...
							if(!empty($_course_fees_data)){
								foreach ($_course_fees_data as $key => $value) {
									...
									$user_courses_exams=$this->strm->get_user_courses_exam(array('user_id'=>$slug_type_id,'user_course_pk_id'=>$value->user_course_id,'user_type'=>'4'),FALSE);
									...
									if($value->user_course_cost_breakup_type=='1'){
										...
										if(!empty($cost_data)){
											...
											if($cost_data->user_course_total_fee>0){
												...
												$total_cost=($cost_total>100000)?number_to_currency($cost_total):number_format($cost_total);//number_to_currency($cost_data->user_course_total_fee);
											}
										...
										if(!empty($cost_data)){
											if(isset($cost_data[0]->user_course_tution_fee_total) && isset($cost_data[0]->user_course_admisssion_fee_total) && isset($cost_data[0]->user_course_reg_fee_total) && isset($cost_data[0]->user_course_exam_fee_total) && isset($cost_data[0]->user_course_other_fee_total)){
												...
												$total_cost=(cost_total>100000)?number_to_currency($cost_total):number_format($cost_total);//number_to_currency($cost_data[0]->total_cost);
											}else{
												$total_cost=($cost_data[0]->total_cost>100000)?number_to_currency($cost_data[0]->total_cost):number_format($cost_data[0]->total_cost);
											}

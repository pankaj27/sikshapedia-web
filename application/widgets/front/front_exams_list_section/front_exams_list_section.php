		...
		if(is_string($segment_1) && $segment_1=='exams'){
			...
			if(!empty($streams)){
				foreach ($streams as $key => $value) {
					...
					$exams=$this->strm->get_indset_exam('exam_stream_id',$value->stream_id,false,array('url_type'=>'exam'),30,'exam_id','ASC',FALSE);
					...
					if(!empty($exams) && count($exams)>=7){
						...
						foreach ($exams as $k => $v) {
							...
							$_exam_logo=$this->sm->__get_user_file('media_disk_path_relative,user_file_type_id,user_storage_type,user_file_type',array('user_file_type_id'=>$v->exam_id,'user_storage_type'=>'exam_logo','user_file_type'=>'6'));
							...
							$_exams[$value->stream_id][]=array(
								...
								'exam_logo'=>$exam_logo,
								'exam_lgo_alt'=>$v->exam_full_name.' - ['.$v->exam_short_name.']'
							);
						}
						...
						$_streams[]=array(
							...
							'stream_icon'=>base_url('public/data/app/app_data/final-course-icon/'.$icon->icon_value),

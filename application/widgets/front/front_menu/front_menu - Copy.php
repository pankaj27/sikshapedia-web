	...
	function run($visible = FALSE) {
    	...
    	if(!empty($top_menues)){
    		foreach ($top_menues as $key => $value) {
    			...
    			$sub_menues=$this->sm->get_menues(array('menu_is_top'=>'2','menu_is_active'=>'1','menu_parent_id'=>$value->menu_id),FALSE,'menu_serial','ASC'); 
                        ...
                        if(!empty($menu_columns)){
                            foreach ($menu_columns as $__k => $__v) {
                                ...
                                if($__v->menu_column_has_menu=='1'){
                                    ...
                                    $_col_image=$this->sm->get_user_file(array('user_file_type_id'=>$__v->menu_column_name_id,'user_storage_type'=>$__v->menu_column_has_image_type));
                                    if(!empty($_col_image)){
     ...
     //    print_obj($footer_ads);die;
     //    die;

        //$str = file_get_contents(FCPATH.'uploads/app/menu.json');

        //$json_menus = json_decode($str, true);

        //  if(!empty($json_menus['menues'])){
        //         foreach ($main_menues['menues'] as $key => $value) {
        //             $_footer_ads_param=array('listing_is_top_menu_id'=>$value['menu_id'],'listing_is_top_menu'=>'1','listing_is_top_menu_type_id'=>$value['menu_link_id']);
        ...
        //$data['top_menues']=$json_menus;

        //print_obj(json_encode($data));die;


        //$data['json_menus']=$json_menus;

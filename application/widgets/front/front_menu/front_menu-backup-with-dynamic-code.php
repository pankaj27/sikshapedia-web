...
class Front_menu extends Widget
{
	...
	function run($visible = FALSE) {
        ...
        $menues_fields='menu_id,menu_name,menu_column_id,menu_is_top,menu_is_active,menu_parent_id,menu_serial,menu_link_id,menu_link_type,menu_link';
    	...
    	if(!empty($top_menues)){
    		foreach ($top_menues as $key => $value) {
    			...
    			if(!empty($sub_menues)){
                       ...
                       $menu_columns=$this->sm->get_menue_columns(array('menu_column_parent_id'=>$v->menu_id,'menu_column_status'=>'1'),FALSE,'menu_column_serial','ASC');
                ...
                foreach ($main_menues['menues'] as $key => $value) {
                    ...
                    if(!empty($_footer_ads)){
                        ...
                        foreach ($_footer_ads as $k => $v){
                            ...
                            $fields='college_user_id,college_name,college_city_id,college_state_id,city_name,state_name';

                            $college_profile_found=$this->im->___get_college_profile_data($fields,array('college_user_id'=>$v->listing_user_id));

                            $college_address=ucwords($college_profile_found->city_name).','.ucwords($college_profile_found->state_name);
                            ...
                            $college_logo=$this->sm->__get_user_file('user_file_type_id,user_storage_type,user_storage_type,media_disk_path_relative',array('user_file_type_id'=>$college_profile_found->college_user_id,'user_storage_type'=>'user_logo'));

...
class PluploadHandler
{
    ...
    protected function rename($tmp_path, $file_path)
    {
        ...
        if (rename($tmp_path, $file_path)) {
            ...
            $file_size=call_user_func($this->conf['cb_filesize'], $file_path);

            $old_name=basename($file_path);
            $extension=substr(strrchr(strtolower($old_name), "."),1);

            $new_name=$this->generateFileName(array('auto', 10), array('name'=>$old_name, 'size'=>$file_size)) . (!empty($extension) ? "." . $extension : "");

            $base_path=pathinfo($file_path, PATHINFO_DIRNAME);

            $realpath=str_replace('\\', '/', $base_path.'/'.$new_name);

            $new_file_path=rename($file_path,$realpath);
            ...
            return array(
                ...
                'path' => $realpath,
                'size' => $file_size,
                'sizeformat'=>$this->formatSize($file_size)
            );
        } else {
            return false;
        }

...
function generate_cipher($PNG_WEB_DIR,$text){
    $PNG_TEMP_DIR=FCPATH.'uploads/temps/';
    
    $filename = $PNG_TEMP_DIR.'test.png';
    $errorCorrectionLevel = 'L';
    $matrixPointSize = 10;
    $secretKey = 'e52232e10ded4587999f4dd135a4a94i';

    // ob_start("callback");
    // ob_get_contents();
    // ob_end_clean();

    $encrypted = Cipher::encrypt($secretKey, $text);

    QRcode::png($encrypted->getData(), $filename, $errorCorrectionLevel, $matrixPointSize, 2);

    $data = file_get_contents($PNG_WEB_DIR.'test.png');

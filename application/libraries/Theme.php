...
class Theme
{
	 ...
	 * @return 	string
	 */
	public function theme_url($uri = '',$dir=null)
	{
		(function_exists('base_url')) OR $this->CI->load->helper('url');
		...
		if($dir!=null){
			...
			return preg_replace(
				'/([^:])(\/{2,})/',
				'$1/',
				"{$url}/content/themes/{$this->view_type}/{$this->theme}/{$uri}"
			);
		}
	...
	{
		return realpath(FCPATH.'content/themes/'.$this->view_type.'/'.$this->theme.'/'.$uri);
	}
	...
	protected function _load_file($type = 'view', $view = '', $data = array(), $return = false)
	{
		switch ($type) {
			...
			case 'partials':
				// prepare all path
				$paths = array(
					build_path(FCPATH, 'content', 'themes', $this->view_type,$this->config['theme'], 'views', '_modules', $this->module, '_partials'),
					build_path(FCPATH, 'content', 'themes', $this->view_type,$this->config['theme'], 'views', '_partials'),
					build_path(APPPATH, $this->view_type,'modules', $this->module, 'views', '_partials'),
					build_path(APPPATH, $this->view_type,'views', '_partials'),
					build_path(VIEWPATH,$this->view_type, '_partials')
				);
			...
			case 'layouts':
				...
				// prepare all path
				$paths = array(
					build_path(FCPATH, 'content', 'themes',$this->view_type, $this->config['theme'], 'views', '_modules', $this->module, '_layouts'),
					build_path(FCPATH, 'content', 'themes',$this->view_type, $this->config['theme'], 'views', '_layouts'),
					build_path(APPPATH, $this->view_type,'modules', $this->module, 'views', '_layouts'),
					build_path(APPPATH, $this->view_type,'views', '_layouts'),
					build_path(VIEWPATH,$this->view_type, '_layouts')
				);
			...
			default:
				...
				// prepare all path
				$paths = array(
					build_path(FCPATH, 'content', 'themes',$this->view_type, $this->config['theme'], 'views', '_modules', $this->module, '_master'),
					build_path(FCPATH, 'content', 'themes',$this->view_type, $this->config['theme'], 'views', '_master'),
					build_path(APPPATH, $this->view_type,'modules', $this->module, 'views', '_master'),
					build_path(APPPATH, $this->view_type,'views', '_master'),
					build_path(VIEWPATH, $this->view_type,'_master')
				);

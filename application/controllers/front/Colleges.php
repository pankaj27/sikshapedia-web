...
class Colleges extends BaseFrontController
{
	...
	public function index_new_123(){

		$current_url=current_url();

		$slug_data=$this->sm->get_slug_urls(array('url_value'=>$current_url));

		// print_obj($slug_data);

		if(!empty($slug_data)){

			$country_id=$slug_data->url_country;
			$state_id=$slug_data->url_state;
			$city_id=$slug_data->url_city;
			$stream_id=$slug_data->url_stream;

			$post=array('length'=>'6','start'=>'0');

			$param=array('country_id'=>$country_id,'status'=>'1');
			...
			if(!empty($colleges)){
				...
				$this->data['data_start']='12';


				$this->data['country_id']=encode_data($country_id);
				$this->data['state_id']=encode_data($state_id);
				$this->data['city_id']=encode_data($city_id);
				$this->data['stream_id']=encode_data($stream_id);
				$this->data['course_id']=encode_data($course_id);
				$this->data['exam_id']=encode_data($exam_id);
				...
				$this->data['page_inner_title']=(!empty($slug_data))?$slug_data->url_page_heading:'';
				$this->data['breadcumb']=(!empty($slug_data))?json_decode($slug_data->url_breadcrumb,TRUE):'';

				//print_obj($this->data);die;

				$this->data['page_id']=$slug_data->url_id;
			...
			if(!empty($country_data)){
				if(is_string($segment_2) && $segment_2=='colleges'){
					...
					if(!empty($current_url_slug)){
						...
						$this->data['_base_url']=base_url($country_data->country_iso_code_2.'/colleges/');
						...
						$this->data['ads_data']=$this->um->get_listing_package_user($ads_param);
						$this->data['utype']='';
	...
	public function indexPublicPages(){
		...
		if(!empty($url_data)){
			...
			$college_data=$this->im->__get_college_profile_data('college_user_id,college_estd_year,college_affiliation_type,college_type,college_utype,college_university_id,college_logo,college_banner,colllege_logo_alt_text,colllege_banner_alt_text,college_has_verified_badge',array('college_user_id'=>$url_data->url_type_id));
			...
			if(!empty($college_data)){
	            ...
	            $college_page_ads=$this->um->__get_listing_package_users($college_data->college_user_id,TRUE);

	            //print_obj($college_page_ads);die;
	            $ads='';

	            if(!empty($college_page_ads)){
	            	$ads_file=$this->sm->get_user_file(array('user_storage_type'=>'ads_image','user_storage_type_2'=>$college_page_ads->listing_id));

	            	if(!empty($ads_file)){
                       $ads=$ads_file->media_disk_path_relative;
                    }
	            }

...
class BaseFrontController extends CI_Controller
{
    ...
    public $current_url='';
  	...
  	function __construct(){
        ...
        if(is_controller(array('Home'))){
          $compress=false;
          $cache_lifetime=0;
        }else if(is_controller(array('Blog'))){
          $compress=true;
          $cache_lifetime=600;
        }else if(is_controller('Colleges')){
          if(is_method('indexPublicPages')){
            $compress=true;
            $cache_lifetime=600;
          }
          else if(is_method('index')){
            $compress=true;
            $cache_lifetime=0;
          }
        }
        else if(is_controller(array('User'))){
          $compress=false;
          $cache_lifetime=0;
        }
        else if(is_controller(array('Datasearch'))){
          $compress=false;
          $cache_lifetime=600;
        }
        else{
        ...
        $country_data=$this->com->__get_country('country_id,country_name,country_phone_code,country_iso_code_2,country_iso_code_4,country_lang,country_default',array('country_default'=>'1'));

        $country_cities=$this->com->__get_city('city_id,city_name,city_country_id,city_state_id',array('city_country_id'=>$country_data->country_id,'city_status'=>'1'),FALSE,'city_name','ASC');


        $this->data['default_country_data']=$country_data;

        $this->data['default_country_cities_data']=$country_cities;
      ...
      $countries=$this->com->__get_country('country_id,country_name,country_phone_code,country_iso_code_2,country_iso_code_3',array('country_status'=>'1'),FALSE,'country_serial','ASC');
        ...
        if(!empty($countries)){
          foreach ($countries as $key => $value) {
            $this->data['countries'][]=array(
              ...
              'country_iso_code_2'=>$value->country_iso_code_2,
